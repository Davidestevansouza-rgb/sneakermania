...
function serviciosItem(it){const s=[];const add=v=>{let vals=v;if(typeof vals==='string'){try{const j=JSON.parse(vals);if(Array.isArray(j))vals=j;}catch{}}for(const x of(Array.isArray(vals)?vals:[vals])){const n=String(x??'').trim();if(n&&!s.includes(n))s.push(n);}};add(it?.tipoServicio);add(it?.tipo_servicio);return s;}
...
export async function verFotosConsulta(id){
  const box=document.getElementById('cu-gallery-'+id);
  if(!box)return;
  const refs=[...box.querySelectorAll('img[data-r2-ref]')].map(img=>img.dataset.r2Ref).filter(Boolean);
...
export async function buscarFichaArticulo(){
 ...
 cont.innerHTML='<div class="hint" style="margin:0 0 8px">'+grupos.size+' orden'+(grupos.size===1?'':'es')+' encontrada'+(grupos.size===1?'':'s')+'</div>'+[...grupos.values()].map(({o,c,items,log})=>{
  ...
  const cards=items.map(({it,regs})=>{const lav=tiene(regs,'lavado'),det=tiene(regs,'detallado','detalle'),ub=ubicacion(it),fs=fotosDe(o,it,regs),estado=it.entregado?'Entregado':(it.estado||o.estado||'Sin estado'),al=alerta(o,it,regs),art=[it.marca,it.modelo,it.descripcion,it.color].filter(Boolean).join(' · ')||'Artículo',servicios=serviciosItem(it);
   ...
   '<div class="cu-services cu-item-services">'+servicios.map(s=>'<span>'+escHtml(s)+'</span>').join('')+'</div><div class="cu-statuses"><div class="cu-status '+(lav?'cu-ok':'')+'"><small>🧼 Lavado</small><strong>'+(lav?'✓ Sí':'Pendiente')+'</strong></div><div class="cu-status '+(det?'cu-ok':'')+'"><small>✨ Detallado</small><strong>'+(det?'✓ Sí':'Pendiente')+'</strong></div><button class="cu-status cu-action" onclick="irBibliotecaConsulta(\''+escAttr(it.id)+'\')"><small>📚 Biblioteca</small><strong>'+escHtml(ub||'Ubicar')+'</strong></button><div class="cu-status"><small>⚙️ Etapa de producción</small><strong>'+escHtml(estado)+'</strong></div></div>'+
   '<div class="cu-gallery cu-inline-gallery" id="cu-gallery-'+escAttr(it.id)+'" aria-hidden="true">'+fs.map(u=>'<img data-r2-ref="'+escAttr(u)+'" src="" alt="Foto del artículo">').join('')+'</div>'+
