...
function mostrarPendientes(row) {
  ...
  files.forEach((file, idx) => {
    ...
    del.type = 'button';
    del.textContent = '×';
    del.title = 'Quitar esta foto pendiente';
...
// Captura ANTES del onchange original. Evita que onFotoFilaItem() suba
// inmediatamente una foto de un artículo ya existente.
  ...
  for (const p of pendientes) {
    ...
    for (const file of p.files) {
      try {
        ...
        orden.extra.fotos.push(foto); // IMPORTANTE: agregar, nunca filtrar/reemplazar anteriores.
        agregadas++;
      } catch (e) {
...
function instalar(intento = 0) {
  ...
  if (typeof original !== 'function') {
    if (intento < 40) setTimeout(() => instalar(intento + 1), 50);
    return;
  }
  ...
  const wrapper = async function(btn, opts = {}) {
    ...
    try {
      await guardarFotosPendientes(ordenId, pendientes);
      showToast('✅ Orden y fotos guardadas');
    } catch (e) {
      console.error('Error guardando fotos pendientes de artículos:', e);
      showToast('La orden se guardó, pero hubo un problema al guardar alguna foto.');
    }
    return resultado;
  };
