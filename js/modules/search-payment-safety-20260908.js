/* ============================================================
   ...
   - Pagos: la validación de sobrepago queda en la capa de datos/servidor;
     este módulo NO intercepta confirmarPagoQR/confirmarPagoEfectivo.
   ============================================================ */
...
function instalarBuscarGaleria() {
  ...
  if (!document.getElementById('galeria-buscar-btn')) {
    ...
    btn.id = 'galeria-buscar-btn'; btn.type = 'button'; btn.className = 'btn btn-primary btn-sm'; btn.textContent = 'Buscar';
    btn.style.marginTop = '8px'; btn.style.marginRight = '8px'; btn.onclick = ejecutarBusquedaGaleria;
    const wrap = input.closest('.combo-wrap'); if (wrap) wrap.appendChild(btn);
  }
  if (!input.dataset.busquedaEnterInstalada) {
    input.dataset.busquedaEnterInstalada = '1';
    input.addEventListener('keydown', ev => { if (ev.key === 'Enter') { ev.preventDefault(); ejecutarBusquedaGaleria(); } });
  }
}
