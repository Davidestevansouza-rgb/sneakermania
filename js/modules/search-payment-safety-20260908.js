...
export function filtrarGaleriaOrdenesFlexible(texto) {
  ...
  const itemMatches = tokens.length
    ? (state.ordenItems || []).filter(it => {
        const o = ordenById(it.ordenId);
        if (!o) return false;
        const haystack = textoBuscableItem(it, o);
        return tokens.every(token => haystack.includes(token));
      }).slice(0, 20)
    : [];

  const opcionTodos = !tokens.length
    ? '<div class="combo-item" onmousedown="seleccionarGaleriaOrden(\'__ALL__\')">👟 <strong>Ver fotos de todas las órdenes</strong></div>'
    : '';
...
// El HTML de Galería llama esta función desde oninput, por eso basta con
// reemplazar la función global sin tocar el módulo de fotos.
window.filtrarGaleriaOrdenes = filtrarGaleriaOrdenesFlexible;

function valorFinalOrden(o) {
  return Math.max(Number(o.precio || 0) - Number(o.descuento || 0), 0);
}
...
function validarNuevoPago(id, inputId) {
  ...
  if (monto <= 0) return true; // deja que la validación original muestre su aviso
...
function validarCorreccionPago(inputId) {
  const id = window.__corregirPagoOrdenIdSafety;
  const o = id ? ordenById(id) : null;
  if (!o) return true;
  const input = document.getElementById(inputId);
  const monto = Number(input && input.value) || 0;
  const valorFinal = valorFinalOrden(o);
  const EPS = 0.000001;
  if (monto - valorFinal > EPS) {
...
// Conserva exactamente el flujo existente de QR/Efectivo/Corrección y solo
// agrega una barrera antes de llamar a las funciones originales.
...
if (typeof originalOpenPagoQR === 'function') {
  window.openPagoQRModal = function(id) {
    window.__pagoQrOrdenIdSafety = id;
    return originalOpenPagoQR(id);
  };
}
if (typeof originalConfirmarPagoQR === 'function') {
  window.confirmarPagoQR = function() {
    if (!validarNuevoPago(window.__pagoQrOrdenIdSafety, 'pago-qr-monto')) return;
    return originalConfirmarPagoQR();
  };
}
if (typeof originalOpenPagoEfectivo === 'function') {
  window.openPagoEfectivoModal = function(id) {
    window.__pagoEfectivoOrdenIdSafety = id;
    return originalOpenPagoEfectivo(id);
  };
}
if (typeof originalConfirmarPagoEfectivo === 'function') {
  window.confirmarPagoEfectivo = function() {
    if (!validarNuevoPago(window.__pagoEfectivoOrdenIdSafety, 'pago-efectivo-monto')) return;
    return originalConfirmarPagoEfectivo();
  };
}
if (typeof originalOpenCorregirPago === 'function') {
  window.openCorregirPagoModal = function(id) {
    window.__corregirPagoOrdenIdSafety = id;
    return originalOpenCorregirPago(id);
  };
}
if (typeof originalGuardarCorreccionPago === 'function') {
  window.guardarCorreccionPago = function() {
    if (!validarCorreccionPago('corregir-pago-monto')) return;
    return originalGuardarCorreccionPago();
  };
}
