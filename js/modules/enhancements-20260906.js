...
function mejorarOrdenUI() {
  ...
  document.querySelectorAll('#orden-items-list .articulo-row').forEach(row => {
    ...
    if (!viejo) return;
    row.dataset.smphoto = '1'; viejo.style.display = 'none';
    const host = viejo.parentElement || row;
    const preview = document.createElement('div'); preview.className = 'sm-preview'; host.appendChild(preview);
...
async function guardarFotoItemAislada(itemId, file) {
  ...
  try {
    ...
    foto.item = item.codigo; foto.itemId = item.id; foto.categoria = 'item_inicial';
    ...
    orden.extra.fotos.push(foto); await persist(); await db.saveOrden(orden);
    logActivity('Agregó foto al artículo ' + item.codigo); showToast('✅ Foto guardada');
...
function mejorarNuevoClienteOrdenUI() {
  const list = document.getElementById('nco-items-list'); if (!list) return;
  list.querySelectorAll('.co-par-row').forEach(row => {
    ...
    if (!viejo) return;
    row.dataset.smnco = '1'; viejo.style.display = 'none'; const rowId = row.dataset.rowId;
    const elegir = (files, input) => { const file = files[0]; if (!archivoImagenValido(file)) { showToast('Selecciona una imagen'); input.value=''; return; } if (window.capturarFotoParCO) window.capturarFotoParCO(rowId, file); input.value = ''; };
    const a = crearFileInput(row, { capture:true }, elegir), b = crearFileInput(row, {}, elegir), c = crearFileInput(row, { accept:null }, elegir);
...
async function recategorizarFotosNuevaOrden(idsAntes) {
  const orden = (state.ordenes || []).find(o => !idsAntes.has(o.id)); if (!orden?.extra?.fotos) return;
  let cambio = false;
  for (const foto of orden.extra.fotos) if (foto.categoria === 'todos_pares' && foto.item) { foto.categoria = 'item_inicial'; const item = (state.ordenItems || []).find(it => it.ordenId === orden.id && it.codigo === foto.item); if (item) foto.itemId = item.id; cambio = true; }
  if (cambio) { await persist(); await db.saveOrden(orden); }
}

function tieneBlanqueamiento(reg) { const item = (state.ordenItems || []).find(x => x.codigo === reg.codigo); return !!item?.registroServicios?.[reg.servicio]?.blanqueamiento || !!item?.blanqueamiento; }
...
function buscarGaleriaMarcaModelo() {
  const caja = document.getElementById('sm-gbrand'); if (!caja) return;
  const q = (caja.querySelector('input')?.value || '').trim().toLowerCase(), out = document.getElementById('sm-gresults');
  if (!q) { out.innerHTML = '<div class="hint">Escribe una marca o modelo.</div>'; return; }
  const corte = fechaHaceDosMeses();
  const resultados = (state.ordenItems || []).filter(it => { const orden = (state.ordenes || []).find(o => o.id === it.ordenId); const fecha = it.fechaIngreso || orden?.fechaIngreso || ''; if (!fecha || fecha < corte) return false; const texto = [it.marca, it.modelo, it.descripcion].filter(Boolean).join(' ').toLowerCase(); return texto.includes(q); }).sort((a,b) => { const oa = (state.ordenes || []).find(o => o.id === a.ordenId), ob = (state.ordenes || []).find(o => o.id === b.ordenId); return (b.fechaIngreso || ob?.fechaIngreso || '').localeCompare(a.fechaIngreso || oa?.fechaIngreso || ''); });
  out.innerHTML = resultados.length ? '<div class="hint" style="margin:8px 0">' + resultados.length + ' coincidencia(s) en los últimos 2 meses.</div>' + resultados.map(renderItemCardHTML).join('') : '<div class="hint" style="margin-top:8px">Sin coincidencias.</div>';
...
function mejorarGaleriaUI() {
  ...
  const box = document.createElement('div'); box.id = 'sm-gbrand'; box.className = 'sm-brandbox';
  box.innerHTML = '<strong>Buscar artículo por marca o modelo</strong><div class="hint">Busca solamente la marca/modelo del artículo registrado. No busca cliente ni número de orden. Muestra los últimos 2 meses.</div><div class="row"><input class="input" placeholder="Ej: Nike Air Force 1"><button class="btn btn-primary btn-sm">Buscar</button></div><div id="sm-gresults"></div>';
  const buscadorPares = document.getElementById('galeria-orden-search')?.closest('.combo-wrap');
  content.parentElement.insertBefore(box, buscadorPares || content);
...
function init() {
  instalarCSS();
  if (typeof window.agregarFotoItem === 'function') { guardarFotoItemAislada.__smOpsWrapped = true; window.agregarFotoItem = guardarFotoItemAislada; }
  envolver('openOrdenModal', mejorarOrdenUI); envolver('agregarFilaItemOrden', mejorarOrdenUI); envolver('sincronizarCantidadPares', mejorarOrdenUI);
  if (typeof window.saveOrden === 'function' && !window.saveOrden.__smOpsSave) { const original = window.saveOrden; const wrapper = function(...args) { const fecha = document.getElementById('orden-fecha-entrega-general')?.value; if (fecha) document.querySelectorAll('#orden-items-list .item-fecha-entrega-input').forEach(x => { x.value = fecha; }); return original.apply(this, args); }; wrapper.__smOpsSave = true; window.saveOrden = wrapper; }
