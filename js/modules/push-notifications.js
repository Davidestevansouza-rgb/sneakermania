...
// Clave PÚBLICA VAPID activa. La privada vive únicamente en configuración
// interna del servidor y nunca debe estar en el frontend ni en el repositorio.
...
async function subscribeToPush() {
  try {
    ...
    // Si la clave VAPID fue rotada, una suscripción vieja ya no es válida para
    // el servidor nuevo. Se reemplaza automáticamente en este dispositivo.
    if (subscription) {
...
export function renderPushPanel() {
  ...
  if (bloqueado) {
    html += `<div class="hint" style="color:var(--red);">⚠️ Las notificaciones están bloqueadas. Permite el acceso desde la configuración del navegador.</div>`;
  } else if (permitido) {
    html += `<div style="color:var(--green);margin-bottom:10px;">✓ Notificaciones activadas — este dispositivo está registrado.</div><button class="btn btn-ghost" onclick="testPushNotification()">Enviar notificación de prueba</button>`;
  } else {
    html += `<button class="btn btn-primary" onclick="activarNotificacionesPush()">Activar notificaciones</button>`;
  }
  return html + `</div>`;
}
