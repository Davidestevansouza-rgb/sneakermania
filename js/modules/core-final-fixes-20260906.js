...
async function prepararFotoGeneral(o) {
  ...
  // Las fotos R2 se muestran correctamente mediante <img> con URL firmada,
  // pero fetch() directo al endpoint privado de R2 requiere CORS del bucket.
  // No hacemos esa descarga automática en segundo plano para evitar errores
  // de consola y bloqueos. WhatsApp sigue abriendo con el texto; la foto queda
  // visible y almacenada normalmente en la orden.
...
function precargarFotosAlAbrir(){
  ['viewOrdenDetalle','openOrdenModal'].forEach(nombre=>{
    ...
    const w=function(id,...rest){const r=original.call(this,id,...rest);return r;};
