...
(function installFotoHotfix() {
  ...
  function normalizarFotosOrden(o) {
    ...
    const unicas = [];
    const seen = new Set();
    for (const f of [...actuales, ...anidadas, ...directas]) {
  ...
  function modalOrdenVisible() {
    ...
    return modal.classList.contains('open') || modal.classList.contains('active') || modal.getAttribute('aria-hidden') === 'false' || (cs.display !== 'none' && cs.visibility !== 'hidden');
  }
  ...
  async function refrescarModalOrden(id) {
    ...
    try {
      ...
      requestAnimationFrame(() => { const c = document.querySelector('#modal-orden .modal-content'); if (c) c.scrollTop = scrollTop; });
    } catch (e) { console.warn('Hotfix fotos: no se pudo refrescar modal', e); }
  }
  ...
  const tryInstall = () => {
    ...
    window.saveOrden = async function (...args) {
      ...
      normalizarTodas();
      if (result && modalOrdenVisible()) await refrescarModalOrden(result); else resolverR2En(document);
      return result;
    };
    if (typeof window.saveOrdenYMantener === 'function' && !window.saveOrdenYMantener.__smFotoKeepOpenV2) {
      const original = window.saveOrdenYMantener;
      ...
      wrapped.__smFotoKeepOpenV2 = true;
