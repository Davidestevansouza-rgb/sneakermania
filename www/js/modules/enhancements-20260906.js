...
function mejorarOrdenUI() {
  ...
  if (cam && gal && !general.querySelector('.sm-general')) {
    ...
    if (caja) {
      ...
      caja.appendChild(crearMenuFoto(() => cam.click(), () => { gal.accept = 'image/*'; gal.click(); }, () => archivo.click(), 'sm-general'));
    }
  }
...
async function guardarFotoItemAislada(itemId, file) {
  ...
  } catch (e) { console.error(e); showToast('Error al guardar la foto'); }
}
...
function mejorarNuevoClienteOrdenUI() {
  ...
  list.querySelectorAll('.co-par-row').forEach(row => {
    ...
    if (!viejo) return;
    row.dataset.smnco = '1'; viejo.style.display = 'none';
    ...
    const a = crearFileInput(row, { capture:true }, elegir), b = crearFileInput(row, {}, elegir), c = crearFileInput(row, { accept:null }, elegir);
...
function tarjetaProduccion(reg) {
  ...
  return '<div class="prod-card">' + foto + '<div style="padding:6px"><div><strong>' + escHtml(reg.empleado || '—') + '</strong>' + (reg.codigo ? ' · <span class="mono">' + escHtml(reg.codigo) + '</span>' : '') + (reg.servicio ? ' · ' + escHtml(reg.servicio) : '') + '</div><div class="hint">' + fmtDate(reg.fecha) + (reg.hora ? ' · ' + escHtml(reg.hora) : '') + '</div>' + (tieneBlanqueamiento(reg) ? '<div style="font-size:11px;font-weight:800;margin-top:3px">Blanqueamiento: Sí</div>' : '') + (reg.observacion ? '<div class="hint" style="white-space:pre-line">📝 ' + escHtml(reg.observacion) + '</div>' : '') + '</div></div>';
}
function renderRangoProduccion() {
  const out = document.getElementById('prod-historial'); if (!out) return;
  const desde = document.getElementById('sm-pdesde')?.value || todayISO(0), hasta = document.getElementById('sm-phasta')?.value || todayISO(0);
  ...
  if (esEmpleado()) registros = registros.filter(propio); else { const empleado = document.getElementById('sm-pemp')?.value || ''; if (empleado) registros = registros.filter(r => r.empleado === empleado); }
...
async function persistirBlanqueamientoSiCorresponde(codigo,servicio,marcado){if(!marcado||!codigo||!servicio)return;const existe=(state.registroPares||[]).some(r=>r.codigo===codigo&&r.servicio===servicio);if(!existe)return;const item=(state.ordenItems||[]).find(x=>x.codigo===codigo);if(!item)return;item.registroServicios=item.registroServicios||{};item.registroServicios[servicio]=item.registroServicios[servicio]||{};if(item.registroServicios[servicio].blanqueamiento===true)return;item.registroServicios[servicio].blanqueamiento=true;await persist();await db.saveOrdenItem(item)}

function fechaCobro(o){return o.fechaPago||o.fechaIngreso||''}
function renderCobrosFinanzas(){const tabla=document.getElementById('finanzas-ordenes-table');if(!tabla)return;const desde=document.getElementById('sm-fdesde')?.value||todayISO(0),hasta=document.getElementById('sm-fhasta')?.value||todayISO(0);if(desde>hasta){tabla.innerHTML='<tbody><tr><td>Rango de fechas inválido.</td></tr></tbody>';return}const ordenes=(state.ordenes||[]).filter(o=>enRango(fechaCobro(o),desde,hasta)).sort((a,b)=>(fechaCobro(b)+String(b.numero).padStart(8,'0')).localeCompare(fechaCobro(a)+String(a.numero).padStart(8,'0')));tabla.innerHTML='<thead><tr><th>Fecha</th><th>Orden</th><th>Cliente</th><th>Precio</th><th>Pagado</th><th>Pendiente</th><th>Método</th><th>Estado</th></tr></thead><tbody>'+ordenes.map(o=>{const total=Number(o.precio)-Number(o.descuento||0),pendiente=Math.max(total-Number(o.pagado||0),0);return '<tr><td data-label="Fecha">'+fmtDate(fechaCobro(o))+'</td><td class="mono" data-label="Orden">#'+escHtml(o.numero)+'</td><td data-label="Cliente">'+escHtml(clienteNombre(o.clienteId))+'</td><td data-label="Precio">'+fmtMoney(total)+'</td><td data-label="Pagado">'+fmtMoney(o.pagado)+'</td><td data-label="Pendiente">'+fmtMoney(pendiente)+'</td><td data-label="Método">'+escHtml(o.metodoPago||'—')+'</td><td data-label="Estado">'+chipPago(o.estadoPago)+'</td></tr>'}).join('')+'</tbody>'}
function mejorarFinanzasUI(){const tabla=document.getElementById('finanzas-ordenes-table');if(!tabla)return;const panel=tabla.closest('.panel');if(panel&&!document.getElementById('sm-frange')){const tools=document.createElement('div');tools.id='sm-frange';tools.className='sm-tools';tools.style.padding='0 20px 12px';tools.innerHTML='<div class="field"><label>Desde</label><input type="date" id="sm-fdesde"></div><div class="field"><label>Hasta</label><input type="date" id="sm-fhasta"></div><button class="btn btn-primary btn-sm" id="sm-fgo">Buscar</button><button class="btn btn-ghost btn-sm" id="sm-ftoday">Hoy</button>';panel.querySelector('.panel-title')?.insertAdjacentElement('afterend',tools);document.getElementById('sm-fdesde').value=todayISO(0);document.getElementById('sm-fhasta').value=todayISO(0);document.getElementById('sm-fgo').onclick=renderCobrosFinanzas;document.getElementById('sm-ftoday').onclick=()=>{document.getElementById('sm-fdesde').value=todayISO(0);document.getElementById('sm-fhasta').value=todayISO(0);renderCobrosFinanzas()}}if(document.getElementById('tab-finanzas')?.classList.contains('active'))renderCobrosFinanzas()}

function fechaHaceDosMeses(){const d=new Date();d.setMonth(d.getMonth()-2);return d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0')+'-'+String(d.getDate()).padStart(2,'0')}
function buscarGaleriaMarcaModelo(){const caja=document.getElementById('sm-gbrand');if(!caja)return;const q=(caja.querySelector('input')?.value||'').trim().toLowerCase(),out=document.getElementById('sm-gresults');if(!q){out.innerHTML='<div class="hint">Escribe una marca o modelo.</div>';return}const corte=fechaHaceDosMeses();const resultados=(state.ordenItems||[]).filter(it=>{const orden=(state.ordenes||[]).find(o=>o.id===it.ordenId),fecha=it.fechaIngreso||orden?.fechaIngreso||'';if(!fecha||fecha<corte)return false;return[it.marca,it.modelo,it.descripcion].filter(Boolean).join(' ').toLowerCase().includes(q)}).sort((a,b)=>{const oa=(state.ordenes||[]).find(o=>o.id===a.ordenId),ob=(state.ordenes||[]).find(o=>o.id===b.ordenId);return(b.fechaIngreso||ob?.fechaIngreso||'').localeCompare(a.fechaIngreso||oa?.fechaIngreso||'')});out.innerHTML=resultados.length?'<div class="hint" style="margin:8px 0">'+resultados.length+' coincidencia(s) en los últimos 2 meses.</div>'+resultados.map(renderItemCardHTML).join(''):'<div class="hint" style="margin-top:8px">Sin coincidencias.</div>';out.querySelectorAll('label[title="Agregar foto de este artículo"]').forEach(x=>x.remove())}
function mejorarGaleriaUI(){const content=document.getElementById('galeria-content');if(!content||document.getElementById('sm-gbrand'))return;const box=document.createElement('div');box.id='sm-gbrand';box.className='sm-brandbox';box.innerHTML='<strong>Buscar artículo por marca o modelo</strong><div class="hint">Busca solamente la marca/modelo del artículo registrado. No busca cliente ni número de orden. Muestra los últimos 2 meses.</div><div class="row"><input class="input" placeholder="Ej: Nike Air Force 1"><button class="btn btn-primary btn-sm">Buscar</button></div><div id="sm-gresults"></div>';content.parentElement.insertBefore(box,content);box.querySelector('button').onclick=buscarGaleriaMarcaModelo;box.querySelector('input').onkeydown=e=>{if(e.key==='Enter')buscarGaleriaMarcaModelo()}}
function sinFotosIndividualesEnGaleria(fn){const backup=[];(state.ordenes||[]).forEach(o=>{if(!Array.isArray(o.extra?.fotos))return;const original=o.extra.fotos,visibles=original.filter(f=>!(f.categoria==='item_inicial'||(f.categoria==='todos_pares'&&f.item)));if(visibles.length!==original.length){backup.push([o,original]);o.extra.fotos=visibles}});try{return fn()}finally{backup.forEach(([o,fotos])=>{o.extra.fotos=fotos})}}

function actualizarResumenMetodosPago(){const resumen=document.getElementById('rep-resumen');if(!resumen)return;[['rep-qr','Ingresos por QR'],['rep-ef','Ingresos en efectivo']].forEach(([id,label])=>{if(!document.getElementById(id)){const card=document.createElement('div');card.className='panel';card.style.cssText='flex:1;min-width:180px';card.innerHTML='<div class="hint">'+label+'</div><div id="'+id+'" style="font-size:22px;font-weight:700">—</div>';resumen.appendChild(card)}});const desde=document.getElementById('rep-desde')?.value||'',hasta=document.getElementById('rep-hasta')?.value||'',ordenes=(state.ordenes||[]).filter(o=>(!desde&&!hasta)||enRango(o.fechaIngreso,desde,hasta));document.getElementById('rep-qr').textContent=fmtMoney(ordenes.reduce((s,o)=>s+Number(o.pagadoQR||0),0));document.getElementById('rep-ef').textContent=fmtMoney(ordenes.reduce((s,o)=>s+Number(o.pagadoEfectivo||0),0))}

function envolver(nombre,despues){const original=window[nombre];if(typeof original!=='function'||original.__smOpsWrapped)return;const wrapper=function(...args){const r=original.apply(this,args);if(r&&typeof r.then==='function')return r.then(async valor=>{await despues(...args);refrescarMejoras();return valor});despues(...args);refrescarMejoras();return r};wrapper.__smOpsWrapped=true;window[nombre]=wrapper}
function init(){instalarCSS();if(typeof window.agregarFotoItem==='function'){guardarFotoItemAislada.__smOpsWrapped=true;window.agregarFotoItem=guardarFotoItemAislada}envolver('openOrdenModal',mejorarOrdenUI);envolver('agregarFilaItemOrden',mejorarOrdenUI);envolver('sincronizarCantidadPares',mejorarOrdenUI);if(typeof window.saveOrden==='function'&&!window.saveOrden.__smOpsSave){const original=window.saveOrden,wrapper=function(...args){const fecha=document.getElementById('orden-fecha-entrega-general')?.value;if(fecha)document.querySelectorAll('#orden-items-list .item-fecha-entrega-input').forEach(x=>{x.value=fecha});return original.apply(this,args)};wrapper.__smOpsSave=true;window.saveOrden=wrapper}envolver('openNuevoClienteOrdenModal',mejorarNuevoClienteOrdenUI);envolver('agregarFilaParCO',mejorarNuevoClienteOrdenUI);if(typeof window.guardarClienteOrden==='function'&&!window.guardarClienteOrden.__smOpsSave){const original=window.guardarClienteOrden,wrapper=async function(...args){const antes=new Set((state.ordenes||[]).map(o=>o.id)),r=await original.apply(this,args);await recategorizarFotosNuevaOrden(antes);refrescarMejoras();return r};wrapper.__smOpsSave=true;window.guardarClienteOrden=wrapper}if(typeof window.registrarPares==='function'&&!window.registrarPares.__smOpsProd){const original=window.registrarPares,wrapper=async function(...args){const codigo=document.getElementById('prod-codigo')?.value.trim()||'',servicio=document.getElementById('prod-servicio')?.value||'',blanqueamiento=!!document.getElementById('prod-blanqueamiento')?.checked,r=await original.apply(this,args);await persistirBlanqueamientoSiCorresponde(codigo,servicio,blanqueamiento);mejorarProduccionUI();return r};wrapper.__smOpsProd=true;window.registrarPares=wrapper}window.renderHistorialProduccion=renderRangoProduccion;envolver('renderFinanzas',mejorarFinanzasUI);envolver('renderReportes',actualizarResumenMetodosPago);envolver('setRepRange',actualizarResumenMetodosPago);['renderGaleria','seleccionarGaleriaOrden','seleccionarGaleriaItem','limpiarFiltroGaleriaItem'].forEach(nombre=>{const original=window[nombre];if(typeof original!=='function'||original.__smOpsGallery)return;const wrapper=function(...args){return sinFotosIndividualesEnGaleria(()=>original.apply(this,args))};wrapper.__smOpsGallery=true;window[nombre]=wrapper});if(typeof window.switchTab==='function'&&!window.switchTab.__smOpsTab){const original=window.switchTab,wrapper=function(tab,...args){const r=tab==='galeria'?sinFotosIndividualesEnGaleria(()=>original.call(this,tab,...args)):original.call(this,tab,...args);refrescarMejoras(tab);return r};wrapper.__smOpsTab=true;window.switchTab=wrapper}document.addEventListener('click',e=>{if(!e.target.closest('.sm-menu'))document.querySelectorAll('.sm-pop.open').forEach(x=>x.classList.remove('open'))});refrescarMejoras()}
