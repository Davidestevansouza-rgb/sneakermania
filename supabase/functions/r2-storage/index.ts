  ...
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      if (!(file instanceof File) || typeof path !== "string" || !path) {
        return jsonResponse({ error: "Faltan datos: file y path son requeridos" }, 400);
      }
      if (file.size <= 0 || file.size > MAX_UPLOAD_BYTES) {
        return jsonResponse({ error: "Archivo demasiado grande o vacio" }, 413);
      }
      if (!ALLOWED_IMAGE_TYPES.has(file.type)) {
        return jsonResponse({ error: "Tipo de imagen no permitido" }, 415);
      }

      const cleanPath = limpiarPath(path);
      if (primerSegmento(cleanPath) !== tenant) {
        return jsonResponse({ error: "No autorizado para escribir en ese path" }, 403);
      }
      ...
      const uploadResp = await aws.fetch(putUrl, {
        method: "PUT",
        body,
    ...
    if (action === "delete") {
      ...
      const cleanPath = limpiarPath(payload.path);
      if (primerSegmento(cleanPath) !== tenant) {
        return jsonResponse({ error: "No autorizado para borrar ese path" }, 403);
      }
      const resp = await aws.fetch(
        `${R2_ENDPOINT.replace(/\/+$/, "")}/${R2_BUCKET}/${cleanPath}`,
        { method: "DELETE" },
      );
      if (!resp.ok && resp.status !== 404) {
        return jsonResponse({ error: "No se pudo borrar el archivo" }, 502);
      }
